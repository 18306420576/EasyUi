...
<html>
  <head>
	...
	<script type="text/javascript">
			...
			$(function(){
				//用户管理中的用户列表和用户功能的点击事件
				$('a[title]').click(function(){ //jquery过滤选择器,a标签中有title属性的才有点击事件
						var src = $(this).attr('title');//获取链接的目标地址
						var title = $(this).html();//获取标题 ---类似于js中的innerhtml方法
						//alert(title);
						if($('#tt').tabs('exists' ,title)){//如果主界面中已经打开了这个窗口
							$('#tt').tabs('select',title);//选中这个窗口
						} else {
							//增加一个窗口
							$('#tt').tabs('add',{   
							    title:title,   //标题
  ...
  <body>
		...
		<div id="cc" class="easyui-layout" fit=true style="width:100%;height:100%;">  
		    <div region="north" title="easyui-layout"  split="false" style="height:100px;">这是顶部区域</div>  
		    <!-- 
