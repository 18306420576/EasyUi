...
<html>
  <head>
	...
	<script type="text/javascript">
		    //此程序有bug----拖拽完成后没有进行排序,即资源管理3拖拽到资源管理2上面刷新后又变成下面了
			var flag ; //判断执行保存还是修改的方法
			$(function(){
				$('#t1').tree({//后台返回的json数据格式有要求
						//发送异步ajax请求, 还会携带一个id的参数 
						url:'ResourceServlet?method=loadTree' ,
						dnd:true ,//设置拖拽效果
						onDrop:function(target , source , point){//拖拽触发的事件
						...
						onContextMenu: function(e,node){//鼠标右击的点击事件
								//禁止浏览器的菜单打开
								e.preventDefault();
						       //让节点处于选中状态
				...
				$('#savebtn').click(function(){
							if(flag == 'add'){
										...
										$.ajax({
											...
											dataType:'json' ,
											success:function(result){
												//刷新节点 ---(此语句很重要,没加的话添加二级菜单会因为获取不到id导致失败)
