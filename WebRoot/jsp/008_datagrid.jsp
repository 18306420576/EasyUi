  ...
  <head>
	...
	<script type="text/javascript">
			$(function(){
					 ...
					 */
					$('#t_user').datagrid({
							idField:'id' ,		//只要创建数据表格 就必须要加 idField(作用非常大)----可以跨页选择数据
							title:'用户列表' ,
							//width:1300 ,
							fit:true,// <!--  fit=true跟随浏览器的变化而变化 -->
							height:450 ,
							url:'UserServlet?method=getList' ,
							fitColumns:true ,  //自动填充各列
							striped: true ,					//隔行变色特性 
							...
							//sortName: 'salary' ,//按照薪水排序
							//sortOrder: 'desc' ,//降序排序
							rowStyler: function(index ,record){//当年龄大于25岁,颜色标红
								 if(record.age > 25){
									 return "background:red";
								 }
							// console.info(index);//往控制台打印信息
						   // console.info(record);
							} ,
							frozenColumns:[[	//冻结列特性 ,不要与fitColumns 特性一起使用 ,设置此列的宽度不变
								{
									field:'ck' ,
									width:50 ,
									checkbox: true
									...
									formatter:function(value , record , index){
										...
										*/
										...
										$.ajax({
											...
											cache:false ,
											async: false ,		//同步请求
											//此处必须设置为同步请求,因为在datagrid初始化的时候,异步请求的返回值还没有返回来
											//必须让程序在这停一下,等待程序的返回值附上值后再执行下面的语句
											data:{id:value},
											dataType:'json' ,
							...
							[
								{
									...
									handler:function(){
										flag = 'edit';
										var arr =$('#t_user').datagrid('getSelections');//api中固定的写法
										if(arr.length != 1){
											$.messager.show({
												title:'提示信息!',
												msg:'只能选择一行记录进行修改!'
				   ...
				   //查询按钮的点击事件
					$('#searchbtn').click(function(){
						$('#t_user').datagrid('load' ,serializeForm($('#mysearch')));//将mysearch表单中的元素序列化传递到后台
					});
					
					//清空按钮的点击事件
					$('#clearbtn').click(function(){
						$('#mysearch').form('clear');
						$('#t_user').datagrid('load' ,{});//调用后台getList方法,传入的参数为空
					});
